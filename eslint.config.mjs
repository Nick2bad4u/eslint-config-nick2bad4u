...
/**
 * @typedef {Record<string, unknown | false | null | undefined>} PluginOverrides
 */

/**
 * @example Import { createConfig } from "eslint-config-nick2bad4u"; import
 * localTypefest from "./plugin.mjs";
 *
 * Export default createConfig({ plugins: { typefest: localTypefest, }, });
 *
 * @typedef {object} Nick2Bad4UEslintConfigOptions
 *
 * @property {string} [rootDirectory] Project root used for TypeScript parser
 ...
 *   remove a plugin's rules by namespace.
 */

/**
 * @param {ReadonlyMap<string, unknown>} pluginOverrideEntries
 * @param {string} pluginName
 * @param {unknown} fallbackPlugin
 *
 * @returns {unknown | null}
 */
const resolvePlugin = (pluginOverrideEntries, pluginName, fallbackPlugin) => {
 ...
 * @returns {string}
 */
const getRulePluginName = (ruleName) => {
    if (ruleName.startsWith("@")) {
        return ruleName.split("/").slice(0, 2).join("/");
    }
 ...
 */
const removeDisabledPluginRules = (configs, disabledPluginNames) => {
    ...
    return configs.map((config) => {
        ...
        if (nextConfig.plugins) {
            nextConfig.plugins = Object.fromEntries(
                Object.entries(nextConfig.plugins).filter(
                    ([pluginName]) => !disabledPluginNames.has(pluginName)
                )
            );
        }
        ...
        if (nextConfig.rules) {
            nextConfig.rules = Object.fromEntries(
                Object.entries(nextConfig.rules).filter(
                    ([ruleName]) =>
                        !disabledPluginNames.has(getRulePluginName(ruleName))
                )
            );
        }
...
export const createConfig = (options = {}) => {
    ...
    const typefest = resolvePlugin(
        pluginOverrideEntries,
        "typefest",
        typefestPlugin
    );
    const etcMisc = resolvePlugin(
        pluginOverrideEntries,
        "etc-misc",
        etcMiscPlugin
    );
